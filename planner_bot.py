...
ADD_TITLE, ADD_CATEGORY, ADD_PRIORITY, ADD_DATE, ADD_TIME, ADD_DESCRIPTION, ENABLE_REMINDER, CHOOSE_REMINDER_TIME, INPUT_REMINDER_TIME = range(9)
EDIT_CHOOSE, EDIT_FIELD = range(9, 11)
...
7. Добавь напоминание (опционально)
...
async def add_task_date(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Обработка кнопок быстрого выбора
    if text == '📅 Сегодня':
        ...
        # Переходим в режим ввода даты вручную
        await update.message.reply_text(
            "📅 Введи дату (ДД.МММ.ГГГГ):\n\n(например: 15.12.2024)",
            reply_markup=ReplyKeyboardMarkup([['⬅️ Отмена']], resize_keyboard=True)
        )
        # Сохраняем флаг что ждем ввода даты
...
async def add_task_description(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Получить описание и спросить про напоминание"""
    text = update.message.text
...
async def enable_reminder(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработчик вопроса про напоминание"""
    text = update.message.text
    ...
    if text == '✅ Да':
        ...
        context.user_data['enable_reminder'] = True
        return CHOOSE_REMINDER_TIME
    else:
        # Сохраняем задачу БЕЗ напоминания
        context.user_data['enable_reminder'] = False
        return await save_task_with_reminder(update, context)

async def choose_reminder_time(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Выбор времени напоминания"""
    text = update.message.text
    
    if text == '⏰ За час':
        context.user_data['reminder_time'] = 'hour'
        return await save_task_with_reminder(update, context)
    elif text == '⏰ За 10 мин':
        context.user_data['reminder_time'] = 'minutes_10'
        return await save_task_with_reminder(update, context)
    elif text == '⏰ Выбрать время':
        await update.message.reply_text(
            "⏰ Введи время напоминания (ЧЧ:ММ):\n\n(например: 09:00)",
            reply_markup=ReplyKeyboardMarkup([['⬅️ Отмена']], resize_keyboard=True)
        )
        return INPUT_REMINDER_TIME
    else:
        # Пропустить напоминание
        context.user_data['reminder_time'] = None
        return await save_task_with_reminder(update, context)

async def input_reminder_time(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == '⬅️ Отмена':
        ...
        try:
            ...
            await update.message.reply_text(
                "❌ Неверный формат. Используй ЧЧ:ММ (например: 09:00)"
            )
            return INPUT_REMINDER_TIME
    
    return await save_task_with_reminder(update, context)

async def save_task_with_reminder(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Сохранить задачу с напоминанием"""
    user_id = update.effective_user.id
    ...
    if success:
        # TODO: Сохранить напоминание в БД
        reminder_msg = ""
        if context.user_data.get('enable_reminder'):
            reminder_type = context.user_data.get('reminder_time')
            if reminder_type == 'hour':
                reminder_msg = " ⏰ Напомню за час!"
            elif reminder_type == 'minutes_10':
                reminder_msg = " ⏰ Напомню за 10 минут!"
            elif reminder_type:
                reminder_msg = f" ⏰ Напомню в {reminder_type}!"
        
        await update.message.reply_text(
            f"✅ Задача добавлена!{reminder_msg}",
            reply_markup=get_main_keyboard()
        )
    else:
        await update.message.reply_text(
...
async def show_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Показываем каждую задачу отдельным сообщением с кнопками
...
def main():
    ...
    add_task_handler = ConversationHandler(
        ...
        states={
            ...
            ENABLE_REMINDER: [MessageHandler(filters.TEXT, enable_reminder)],
            CHOOSE_REMINDER_TIME: [MessageHandler(filters.TEXT, choose_reminder_time)],
            INPUT_REMINDER_TIME: [MessageHandler(filters.TEXT, input_reminder_time)],
        },
